...
CDR_HEADER_SIZE = 4
DEFAULT_TOPICS = ("**/diagnostics/hardware",)
running = True
...
def read_diagnostic_status(reader):
    return {
...
def decode_diagnostic_array_payload(payload):
    ...
    for aligned in (True, False):
        for base_offset in (CDR_HEADER_SIZE, 0):
            try:
                ...
                status = reader.read_sequence(read_diagnostic_status, 512, "diagnostic status")
                return {
                    "header": header,
                    "stamp": header["stamp"],
                    "status": status,
...
def parse_namespace(key):
    ...
    suffix = "/diagnostics/hardware"
    if normalized.endswith(suffix):
        return normalized[:-len(suffix)]
    if normalized == "diagnostics/hardware":
        return ""
    return ""
...
def main():
    ...
    key_exprs = normalize_topics(args.topic if args.topic else DEFAULT_TOPICS)

    if not key_exprs:
        emit({"type": "error", "message": "no hardware diagnostics topics configured"})
        return 1
    ...
    def on_sample(sample):
        ...
        message = decode_diagnostic_array_payload(payload)
        if message is None:
