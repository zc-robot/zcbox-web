...
def read_state(reader):
    return {"id": reader.read_uint8(), "label": reader.read_string()}


def decode_transition_event_payload(payload):
    ...
    for aligned in (True, False):
        for base_offset in (CDR_HEADER_SIZE, 0):
            try:
                ...
                stamp = {"sec": reader.read_int32(), "nanosec": reader.read_uint32()}
                transition = read_state(reader)
                start_state = read_state(reader)
                goal_state = read_state(reader)
                return {
                    "stamp": stamp,
...
def normalize_topics(namespace, topics):
    ...
    for raw_topic in topics:
        ...
        if not topic:
            continue
        key_expr = topic if not normalized_namespace or topic.startswith(f"{normalized_namespace}/") else f"{normalized_namespace}/{topic}"
...
def main():
    ...
    key_exprs = normalize_topics(args.namespace, args.topic)
    if not key_exprs:
        emit({"type": "error", "message": "no lifecycle transition topics configured"})
        return 1

    try:
        import zenoh
    except Exception as error:
        emit({"type": "error", "message": f"Failed to import eclipse-zenoh: {error}"})
        return 1
    ...
    def on_sample(sample):
        ...
        event = decode_transition_event_payload(payload)
        if event is None:
            now = time.monotonic()
            if now - last_decode_error_at.get(key, 0) > 2:
                emit({"type": "decode-error", "key": key, "byteLength": len(payload)})
                last_decode_error_at[key] = now
            return
        emit({"type": "transition-event", "key": key, **event})

    try:
        emit({"type": "status", "state": "connecting", "endpoint": f"tcp/{args.host}:{args.port}", "keys": key_exprs})
        session = zenoh.open(config)
        subscribers = [session.declare_subscriber(key, on_sample) for key in key_exprs]
        emit({"type": "status", "state": "subscribed", "keys": key_exprs})
        while running:
            time.sleep(0.2)
    except Exception as error:
