...
def decode_bond_status_payload(payload):
        ...
        for base_offset in (CDR_HEADER_SIZE, 0):
            try:
                ...
                return {
                    ...
                    "instanceId": reader.read_string(),
                    "active": reader.read_bool(),
                    "heartbeatTimeout": reader.read_float32(),
                    "heartbeatPeriod": reader.read_float32(),
                }
...
def main():
    ...
    if not key_exprs:
        emit({"type": "error", "message": "no bond topics configured"})
        return 1
    ...
    def on_sample(sample):
        ...
        status = decode_bond_status_payload(payload)
        if status is None:
            now = time.monotonic()
